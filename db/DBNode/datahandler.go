...
func (entry *DBEntry) Hash() uint64 {
	hash := fnv.New64()
	hash.Write([]byte(entry.ShortURL))
	return hash.Sum64()
}
...
// returns the long url for the corrosponding short url (if it exists)
func DB_Read(shortURL string) *DBEntry {
	if value, found := g_dataCache.Find(shortURL); found {
		return &value
	}

	entry := DBEntry{ShortURL: shortURL, LongURL: ""}
	...
	if hasDataLocally {
		savedEntry := Sqlite_Read(entry.ShortURL)
		if savedEntry != nil {
	...
	for _, ownerID := range ownerNodes {
		if ownerID != uint32(g_id) {
			savedEntry := GetDataFromNode(shortURL, ownerID)
			if savedEntry != nil {
				return savedEntry
			}

			log.Printf("Failed to get url from node %d, trying a different node\n", ownerID)
		}
	}

	log.Printf("Failed to find long url for %s\n", shortURL)
	return nil
}
...
func DB_LocalWrite(data DBEntry) bool {
	...
	if _, found := g_dataCache.Find(data.ShortURL); found {
...
func DB_LocalRead(shortURL string) *DBEntry {
	if value, found := g_dataCache.Find(shortURL); found {
		return &value // cache hit
	}

	return Sqlite_Read(shortURL)
}

func DB_LocalDelete(data DBEntry) {
	g_dataCache.Delete(data.ShortURL)
	Sqlite_Delete(data.ShortURL)
}
