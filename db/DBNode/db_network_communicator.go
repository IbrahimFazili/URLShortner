...
func (node *DBNode) Send(data DBEntry, numTries uint16) {
	for numTries > 0 {
		_, err := http.Post(fmt.Sprintf("%s/internal/set?short=%s&long=%s", node.Addr, url.QueryEscape(data.ShortURL), url.QueryEscape(data.LongURL)), "application/json", http.NoBody)
		if err != nil {
...
func GetDataFromNode(shortURL string, id uint32) *DBEntry {
	for _, node := range g_dbNetwork.Nodes {
		if node.ID == int32(id) {
			res, err := http.Get(fmt.Sprintf("%s/internal/get?short=%s", node.Addr, url.QueryEscape(shortURL)))

			if err != nil {
				log.Printf("GetDataFromNode: Failed to fetch url=%s from node=%d\n", shortURL, id)
				return nil
			}
